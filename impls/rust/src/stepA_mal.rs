...
fn eval_ast(ast: Rc<MalVal>, env: Rc<RefCell<Env>>) -> MalResult {
    match ast.as_ref() {
        MalVal::Symbol(symbol) => env.as_ref().borrow().get(symbol).ok_or_else(|| {
            MalError::Throw(Rc::new(MalVal::String(format!("'{symbol}' not found"))))
        }),
        MalVal::List(list) => {
            let mut buffer = Vec::new();
            for v in list {
            ...
            MalVal::List(list) => {
                if let MalVal::Symbol(symbol) = list[0].as_ref() {
                    match symbol.as_str() {
                        ...
                        "try*" => {
                            let eval_catch = |v: Rc<MalVal>| -> (Rc<MalVal>, Rc<RefCell<Env>>) {
                                if let MalVal::List(catch) = list[2].as_ref() {
                                    if let (MalVal::Symbol(flag), MalVal::Symbol(symbol)) =
                                        (catch[0].as_ref(), catch[1].as_ref())
                                    {
                                        if let "catch*" = flag.as_str() {
                                            let mut n_env = Env::new(env.clone());
                                            n_env.set(symbol.clone(), v);
                                            return (
                                                catch[2].clone(),
                            ...
                            match eval(list[1].clone(), env.clone()) {
                                Ok(v) => return Ok(v),
                                Err(MalError::Throw(v)) if list.len() > 2 => {
                                    (ast, env) = eval_catch(v);
                                    continue;
                                }
                                Err(e) => match e {
                                    MalError::Unbalance(_) if list.len() > 2 => {
                                        (ast, env) =
                                            eval_catch(Rc::new(MalVal::String(e.to_string())));
                                        continue;
                                    }
                                    MalError::Continue => return Ok(Rc::new(MalVal::Nil)),
                                    _ => return Err(e),
                                },
                            };
                        }
...
fn main() {
    ...
    env.set(
        ...
        Rc::new(MalVal::String("rust".to_string())),
    );
